...
from llmpony.llm.functions.ponyge.txt_individuals_from_json import txt_population
from llmpony.llm.functions.ponyge.individual_formatter import extract_all_methods_from_imports
import ast
from llmpony.llm.functions.imports_and_prompt import extract_prompt_info_with_keybert, extract_numbers_from_string
...
from llmpony.llm.functions.json_data_io import read_json, create_dir_path_string
import traceback
from llmpony.pony.utilities.stats.file_io import create_results_folder_path
...
def create_grammar_from(
    ...
    base_grammar_path = os.path.join("PonyGE2/grammars/dynamic/", bnf_type, "")
    if not os.path.isdir(base_grammar_path):
        os.makedirs(base_grammar_path, exist_ok=True)
    ...
    if not only_impr:
        with open("PonyGE2/grammars/dynamic/dynamic.bnf" if bnf_type != COMPLETEBNF_AS_STRING else "PonyGE2/grammars/dynamic/pybnf_spaces.bnf", 'rb') as source_file, open(actual_grammar_path.replace(".json", ".bnf"), 'wb') as dest_file:
...
def create_params_file(
    ...
    improvement_dir: str = os.path.join("PonyGE2/parameters/improvements", "")
    if not os.path.isdir(improvement_dir):
        os.makedirs(improvement_dir, exist_ok=True)
    with open(os.path.join(improvement_dir, "progimpr_base.txt"), 'r') as file:
