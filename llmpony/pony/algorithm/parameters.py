...
def create_default_params_dict():
    return {
...
params = create_default_params_dict()
...
def clear_and_restore_params_dict(params: dict):
    params.clear()
    params.update(create_default_params_dict())
    params['WORST_POSSIBLE_FITNESS_GLOBALLY_EVER'] = params['WORST_POSSIBLE_FITNESS'] * 1000 * 5
...
def set_params(command_line_args, create_files=True):
    ...
    from llmpony.pony.utilities.algorithm.initialise_run import initialise_run_params
    from llmpony.pony.utilities.algorithm.initialise_run import set_param_imports
    from llmpony.pony.utilities.fitness.math_functions import return_one_percent
    from llmpony.pony.utilities.algorithm.command_line_parser import parse_cmd_args
    ...
    if '--grammar_file' in command_line_args and os.path.exists(os.path.join('PonyGE2/grammars', command_line_args[command_line_args.index('--grammar_file') + 1].replace('.bnf', '_complete_dynamic.bnf'))):
    ...
    if 'PARAMETERS' in cmd_args:
        load_params(path.join("PonyGE2", "parameters", cmd_args['PARAMETERS']))
    ...
    if params['LOAD_STATE']:
        # Load run from state.
        from llmpony.pony.utilities.algorithm.state import load_state

        # Load in state information.
        individuals = load_state(params['LOAD_STATE'])

        # Set correct search loop.
        from llmpony.pony.algorithm.search_loop import search_loop_from_state
        ...
        params['BNF_GRAMMAR'] = grammar.Grammar(
            path.join("PonyGE2", "grammars", params['GRAMMAR_FILE']))
        ...
        if params['TARGET_SEED_FOLDER']:
            ...
            from llmpony.pony.operators.initialisation import load_population
