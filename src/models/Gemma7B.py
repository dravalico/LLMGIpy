...
class Gemma7B(AbstractLanguageModel):
    ...
    def _load_model(self) -> Any:
        ...
        self.__tokenizer = AutoTokenizer.from_pretrained("google/gemma-1.1-7b-it", token=os.getenv('HUGGING_TOKEN'))
    ...
    def __extract_response(response: str) -> str:
        return response[response.index(Gemma7B.__SECOND_PART) + len(Gemma7B.__SECOND_PART): len(response):]
